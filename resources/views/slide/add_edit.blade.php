@php
    $web_title = isset($slide) ? 'Төрөл засах' : 'Төрөл нэмэх';
@endphp
<x-app-layout>
    <style>
        #preview {
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mx-auto max-w-6xl">
                <form method="post" class="mx-auto max-w-5xl"
                    action="{{ isset($slide) ? route('slide-update', $slide->id) : route('slide-store') }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <h1 class="text-2xl/8 font-semibold text-zinc-950 sm:text-xl/8 dark:text-white">Слайд
                        {{ isset($slide) ? 'засах' : 'нэмэх' }}</h1>
                    ...
                    <section class="grid gap-x-8 gap-y-6 sm:grid-cols-2">
                        <div class="space-y-1">
                            <h2 class="text-base/7 font-semibold text-zinc-950 sm:text-sm/6 dark:text-white">Манга
                            </h2>
                            <p data-slot="text" class="text-base/6 text-zinc-500 sm:text-sm/6 dark:text-zinc-400">Тухайн
                                слайдны мангаг сонгох</p>
                        </div>
                        <div>
                            <span data-slot="control"
                                ...
                                <select aria-label="Currency" name="manga_id"
                                    ...
                                    @foreach ($mangas as $manga)
                                        <option value="{{ $manga->id }}"
                                            {{ isset($slide) && $slide->manga_id == $manga->id ? 'selected' : '' }}>
                                            {{ $manga->title }}</option>
                                    @endforeach
                                </select>
                    ...
                    <section class="grid gap-x-8 gap-y-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <div id="preview">
                                {!! isset($slide) ? '<img src="data:image/webp;base64,' . $slide->image . '" alt="">' : 'No image uploaded' !!}
                            </div>
                        </div>
                    </section>
                    ...
                    <div class="flex justify-end gap-4"><button type="reset"
                            ...
                            data-headlessui-state=""><span
                                class="absolute left-1/2 top-1/2 size-[max(100%,2.75rem)] -translate-x-1/2 -translate-y-1/2 [@media(pointer:fine)]:hidden"
                                aria-hidden="true"></span>{{ isset($slide) ? 'Хадгалах' : 'Нэмэх' }}</button></div>
                </form>
            </div>
        </div>
    </div>
