@php
    $web_title = 'Үйлчилгээний эрхийн жагсаалт';
@endphp
<x-app-layout>
    <div class="py-12">
                                        ...
                                        @foreach ($memberships as $key => $membership)
                                            <tr>
                                                <td class="px-4 py-4 text-zinc-500 dark:text-zinc-400 border-b">
                                                    {{ $memberships->currentPage() * $memberships->perPage() - $memberships->perPage() + $key + 1 }}
                                                </td>
                                                <td class="px-4 py-4 border-b">
                                                    <a href="{{ route('chapter-list', $membership->id) }}">
                                                        <div class="flex items-center gap-2">

                                                            <span class="font-medium">{{ $membership->title }}</span>
                                                        </div>
                                                    </a>
                                                </td>
                                                ...
                                                <td class="px-4 py-4 text-right border-b">
                                                    <span
                                                        class="inline-flex items-center gap-x-1.5 rounded-md px-1.5 py-0.5 text-sm/5 font-medium {{ $membership->type == 'ongoing' ? 'bg-green-700/90 text-white' : 'bg-lime-400/20 text-lime-700' }} dark:bg-lime-400/10 dark:text-lime-300">
                                                        {{ $membership->type == 'premium' ? 'Premium' : 'Энгийн' }}
                                                    </span>
                                                </td>
                                                <td class="px-4 py-4 text-right border-b">
